    ...
    return (
        <div className="bg-base-300 rounded-lg border-2 border-gray-600">
            ...
            <div className="p-6">
                ...
                <p className='py-4'>{description.length > 100 ? description.slice(0, 100) + '...' : description}</p>
                <h4 className='text-lg font-semibold'>For Age: {forAge}</h4>
                <h4 className='text-lg font-semibold'>Time Required: {timeRequired}s</h4>
                <button onClick={() => handleAddToList(exercise)} className="w-full bg-primary text-white p-3 mt-3 hover:bg-green-600 disabled:bg-secondary text-2xl rounded-md font-semibold">
                    Add to list
                </button>
